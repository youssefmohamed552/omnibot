...
publish_est_odom(){
  ...
  est_odom.pose.pose.position.x = m_est_mu(0);
  est_odom.pose.pose.position.y = m_est_mu(1);
  est_odom.pose.pose.orientation = yaw_to_quaternion(m_est_mu(2));
...
step(const double& dt){
  ...
  Gt(0, 0) = 1;
  Gt(1, 1) = 1;
  Gt(0, 2) = -((m_u(0)/m_u(1)) * cos(theta)) + ((m_u(0)/m_u(1)) * cos(theta + (m_u(1) * dt)));
  Gt(1, 2) = -((m_u(0)/m_u(1)) * sin(theta)) + ((m_u(0)/m_u(1)) * sin(theta + (m_u(1) * dt)));
  Gt(2, 2) = 1;

  Eigen::MatrixXd Vt = Eigen::MatrixXd::Zero(3, 2);
  Vt(0, 0) = (-(sin(theta)) + (sin(theta + (m_u(1) * dt)))) / m_u(1);
  Vt(1, 0) = (cos(theta) - cos(theta + (m_u(1) * dt))) / m_u(1);
  Vt(0, 1) = ((m_u(0) * (sin(theta) - sin(theta + (m_u(1) * dt)))) / (m_u(1) * m_u(1))) + ((m_u(0) * (cos(theta + (m_u(1) * dt))) * dt) / m_u(1));
  Vt(1, 1) = -((m_u(0) * (cos(theta) - cos(theta + (m_u(1) * dt)))) / (m_u(1) * m_u(1))) + ((m_u(0) * (sin(theta + (m_u(1) * dt))) * dt) / m_u(1));
  Vt(2, 1) = dt;
  ...
  dmu(0) = -((m_u(0) / m_u(1)) * sin(theta)) + ((m_u(0) / m_u(1)) * sin(theta + (m_u(1) * dt)));
  dmu(1) = ((m_u(0) / m_u(1)) * cos(theta)) - ((m_u(0) / m_u(1)) * cos(theta + (m_u(1) * dt)));
  dmu(2) = m_u(1) * dt;
  ...
  for(const auto& obs: m_z.observations){
    ...
    z_t(0) = sqrt(q);
    z_t(1) = atan2(m_m[j].y - mu_t(1), m_m[j].x - mu_t(0));
    z_t(2) = j;
